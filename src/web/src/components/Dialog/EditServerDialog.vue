...
// 创建不需要 uuid 剔除 uuid
type CreateServerType = Omit<ServerType, 'uuid'>
...
const emit = defineEmits<{
  confirm: [data: CreateServerType | ServerType]  // 新建或编辑
...
const formData = ref<FormData>(createDefaultForm())
const errorMsg = ref<string>('')
const dialogRef = ref<HTMLDialogElement | null>(null)
// 判断是否为编辑模式
const isEditMode = computed(() => Boolean(props.server))
...
const fillForm = (): void => {
  if (props.server) {
    formData.value = props.server ? { ...createDefaultForm(), ...props.server } : createDefaultForm()
  }
  else {
    resetForm()
  }
      ...
      <div class="flex flex-col">
        ...
        <fieldset class="fieldset bg-base-200 border-base-300 rounded-box border p-3 pt-1">
          ...
          <label class="input form-control w-full">
            ...
            <input type="text" class="grow" v-model="formData.name" placeholder="请输入服务器名称" @input="clearError()" />
        ...
        <label class="mt-2">
          <input type="checkbox" v-model="formData.forceUtf8Mode" class="checkbox checkbox-primary" />
          启用强兼容UTF8模式(中文乱码时勾选)
        </label>

        <label class="mt-2">
          <input type="checkbox" v-model="formData.usePty" class="checkbox checkbox-primary" />
          启用伪终端仿真模拟
        </label>
