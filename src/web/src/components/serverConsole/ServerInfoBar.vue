  ...
  <div class="flex justify-between mb-2">
    ...
    <div class="flex gap-2">
      <button
        ...
        :class="[server.isRunning ? 'btn-disabled' : '']"
        :disabled="actionLoading"
        @click="$emit('start')"
      >
        <span v-show="actionLoading" class="loading loading-spinner size-[1.2em]"></span>
        <svg v-show="!actionLoading" class="size-[1.2em]" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
          <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            d="M5 5a2 2 0 0 1 3.008-1.728l11.997 6.998a2 2 0 0 1 .003 3.458l-12 7A2 2 0 0 1 5 19z" />
        </svg>
        启动
      </button>
      <button class="btn btn-outline btn-error" :disabled="actionLoading" @click="$emit('stop')">
        <span v-show="actionLoading" class="loading loading-spinner size-[1.2em]"></span>
        <svg v-show="!actionLoading" class="size-[1.2em]" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
